...
void AddMessageToChatLog(std::string_view message, Player *player, UiFlags flags)
{
	...
	if (player == nullptr) {
		...
		std::string text = WordWrapString(prefix + std::string(message), ContentTextWidth, devilution::GameFont12, 1, false);
...
void DrawChatLog(const Surface &out)
{
	...
	for (int i = 0; i < numLines; i++) {
		...
		DrawStringWithColors(out, line, args, { { (sx), contentY + i * lineHeight }, { ContentTextWidth, 0 } }, UiFlags::ColorWhite, /*spacing=*/1, lineHeight);
	}
